...
Vec3 Mininec3Compat::pointFromP(double P, int wireIndex, int Kimg) const
{
    ...
    auto nodePos = [&](int nodeId) -> Vec3 {
        Vec3 v(st_.X.at(nodeId), st_.Y.at(nodeId), st_.Z.at(nodeId));
        v.z *= (double)Kimg;
        return v;
    };
    ...
    {
        if (i >= N - 1) i = N - 2; // ensure i+1 exists
        Vec3 a = nodePos(nodes.at(i));
        Vec3 b = nodePos(nodes.at(i + 1));
        return (a + b) * 0.5;
    }
...
{
    // Map wire indices:
    // Observation wire = W%(I) in BASIC
    // Source wire = P4 in BASIC is segment id, but the wire index is implied.
    // In the matrix code, P1 belongs to obs pulse wire, P2/P3 belong to source pulse wire.
    //
    // For now we assume:
    //   obs wire = st_.Wpulse[current I]
    //   src wire = st_.Wpulse[current J]
    //
    // To keep 1:1, we pass the correct wire index when calling this function.
    // For now, we cheat by using wireIndex = st_.Wpulse[0] (you will pass correct later).

    (void)P4; // P4 used later for radius/length special cases

    // NOTE: We need the wire index for obs and src.
    // We'll store them in scratch (set before calling psi).
    // int obsWire = st_.WpulseScratchObs;
    // int srcWire = st_.WpulseScratchSrc;

    // Vec3 R1 = pointFromGlobalP(P1, obsWire, Kimg);
    // Vec3 R2 = pointFromGlobalP(P2, srcWire, Kimg);
    // Vec3 R3 = pointFromGlobalP(P3, srcWire, Kimg);

    Vec3 R1 = pointFromP_Basic(P1, Kimg);
    Vec3 R2 = pointFromP_Basic(P2, Kimg);
    Vec3 R3 = pointFromP_Basic(P3, Kimg);


    Vec3 V = R3 - R2; // segment vector

    double T1 = 0.0;
    double T2 = 0.0;

    // Integrate from t in [-1,1], point = (R2+R3)/2 + t*V/2
    Vec3 mid = (R2 + R3) * 0.5;
    Vec3 half = V * 0.5;
    ...
    for (int i = 0; i < 7; ++i)
    {
        ...
        double Rmin = (st_.SRM > 0.0) ? st_.SRM : 1e-6;
        if (R < Rmin) R = Rmin;

        double kR = st_.W * R;
        double c = std::cos(kR) / R;
        double s = -std::sin(kR) / R;

        T1 += W7[i] * c;
        T2 += W7[i] * s;
    }

    // scale by segment length / 2 (Jacobian)
    double segLen = std::sqrt(V.x*V.x + V.y*V.y + V.z*V.z);
    double scale = segLen * 0.5;
...
Vec3 Mininec3Compat::pointFromGlobalP(double Pglobal, int wireIndex, int Kimg) const
{
    ...
    // Convert global P to local P for this wire
    double Plocal = Pglobal - (double)st_.wireBaseP.at(wireIndex);

    // Now Plocal should be in [0 .. N-1] (+ half steps)
    double iFloor = std::floor(Plocal + 1e-12);
    double frac   = Plocal - iFloor;
    int i = (int)iFloor;

    // No clamp needed if wireBaseP is correct,
    // but we keep a hard assert-like guard for debugging:
    if (i < 0 || i >= N)
    {
        // If this triggers, your P mapping is wrong.
        // You can replace with clamp during debugging.
        throw std::out_of_range("pointFromGlobalP: Plocal out of range");
    }
    ...
    {
        if (i + 1 >= N)
            throw std::out_of_range("pointFromGlobalP: midpoint beyond end");
        Vec3 a = nodePos(nodes.at(i));
        Vec3 b = nodePos(nodes.at(i + 1));
        return (a + b) * 0.5;
    }
